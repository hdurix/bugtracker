...
import com.hippo.tracker.domain.Ticket;
...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    @Query(value = "select distinct ticket from Ticket ticket left join fetch ticket.tags",
        countQuery = "select count(distinct ticket) from Ticket ticket")
    Page<Ticket> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct ticket from Ticket ticket left join fetch ticket.tags")
    List<Ticket> findAllWithEagerRelationships();

    @Query("select ticket from Ticket ticket left join fetch ticket.tags where ticket.id =:id")
